class App {
    ...
    initPortletCard() {
        document.querySelectorAll('[data-action="card-close"]').forEach(a => {
            a.addEventListener("click", e => {
                ...
                t && (t.style.transition = "opacity 0.3s ease",
                t.style.opacity = "0",
        ...
        document.querySelectorAll('[data-action="card-toggle"]').forEach(o => {
            o.addEventListener("click", e => {
                ...
                setTimeout(function() {
                    ...
                    a.style.height = "auto"
                }, 350)) : (a.style.height = i + "px",
                a.style.overflow = "hidden",
                a.offsetHeight,
                e.classList.add("card-collapsed"),
                a.style.height = "0"),
                t && (t.classList.toggle("ti-chevron-up"),
        ...
        document.querySelectorAll('[data-action="card-refresh"]').forEach(i => {
            i.addEventListener("click", e => {
                e.preventDefault();
                var t, e = i.closest(".card");
                let a = e.querySelector(".card-overlay");
                a || ((a = document.createElement("div")).className = "card-overlay",
                (t = document.createElement("div")).className = "spinner-border text-primary",
                a.appendChild(t),
                e.appendChild(a)),
    ...
    initMultiDropdown() {
        document.querySelectorAll(".dropdown-menu a.dropdown-toggle").forEach(a => {
    ...
    initCounter() {
        ...
        let t = new IntersectionObserver( (e, n) => {
            e.forEach(s => {
                if (s.isIntersecting) {
                    let e = s.target, t = e.getAttribute("data-target").replace(/,/g, ""), a = (t = parseFloat(t),
                    0), i, o = (i = Number.isInteger(t) ? Math.floor(t / 25) : t / 25,
                    () => {
                        a < t ? ((a += i) > t && (a = t),
                        e.innerText = r(a),
                        requestAnimationFrame(o)) : e.innerText = r(t)
                    }
                    );
                    function r(e) {
    ...
    initLeftSidebar() {
        ...
        if (s) {
            ...
            s.querySelectorAll("a").forEach(t => {
                if (t.href === e) {
                    s.querySelectorAll("a.active, li.active, .collapse.show").forEach(e => {
...
class LayoutCustomizer {
    ...
    monochromeMode() {
        ...
        e && e.addEventListener("click", () => {
            ...
            this.config.monochrome ? this.html.classList.add("monochrome") : this.html.classList.remove("monochrome"),
            this.setSwitchFromConfig()
        }
        )
    }
...
let radios = document.querySelectorAll('.thememode-dropdown .dropdown-item input[type="radio"]')
  , items = document.querySelectorAll(".thememode-dropdown .dropdown-item")
  , saved = sessionStorage.getItem("theme");
saved && radios.forEach(e => e.value === saved && (e.checked = !0,
e.closest(".thememode-dropdown .dropdown-item").classList.add("active"))),
radios.forEach(e => e.addEventListener("change", () => {
    items.forEach(e => e.classList.remove("active")),
    e.checked && (e.closest(".thememode-dropdown .dropdown-item").classList.add("active"),
...
class I18nManager {
    constructor({defaultLang: e="en", langPath: t="assets/data/translations/", langImageSelector: a="#selected-language-image", langCodeSelector: i="#selected-language-code", translationKeySelector: o="[data-lang]", translationKeyAttribute: s="data-lang", languageSelector: r="[data-translator-lang]"}={}) {
    ...
    updateSelectedImage() {
        ...
        e && (document.querySelectorAll(".dropdown-item.active").forEach(e => {
            e.classList.remove("active")
        }
        ),
        e = e.closest(".dropdown-item")) && e.classList.add("active")
    }
    updateSelectedCode() {
...
class CustomChartJs {
    ...
    render() {
        try {
            ...
            this.element ? (this.chart && this.chart.destroy(),
            ...
            , 200))) : console.warn(`CustomChartJs: No element found for selector '${this.selector}'`)
        } catch (e) {
            console.error(`CustomChartJs: Render error for '${this.selector}'`, e)
        }
    }
    static rerenderAll() {
        for (var e of CustomChartJs.instances)
            e.render()
    }
    static reSizeAll() {
        for (var e of CustomChartJs.instances)
